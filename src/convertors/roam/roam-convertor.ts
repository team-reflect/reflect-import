...
export class RoamConvertor implements Convertor {
  ...
  private generateList(note: RoamNote): DOM {
    ...
    const listItems = note.children.map((child) =>
  ...
  private generateListItem(noteString: RoamNoteString): DOM {
    let string = this.convertRoamTagsToBacklinks(
      noteString.string?.trim() ?? ""
    );

    let checked: boolean | undefined;

    string = string.replace(
      LINKED_TODO_UNFINISHED_STRING,
      TODO_UNFINISHED_STRING
    );
